...
The timeline track is a filmstrip of thumbnails, built in **one ffmpeg pass**:

```
ffmpeg -hwaccel auto -skip_frame nokey -an -i <src> \
  -fps_mode passthrough \
  -vf "select='isnan(prev_selected_t)+gte(t-prev_selected_t,<step>)',
       [transpose,] scale=-2:<h> [,tonemap]" \
  -q:v 4 -progress pipe:1 -nostats <dir>/t%04d.jpg

where <step> = duration / thumbnailCount
```

`-skip_frame nokey` is what makes it fast: the decoder discards non-keyframes
before reconstructing them, and the file is opened and demuxed once.

**Do not go back to one process per thumbnail.** The first version did that —
N spawns, each seeking independently — and cost ~10 s for a 2-minute 1080p
10-bit HEVC clip against ~1 s for the single pass. Process startup and repeated
demuxing dominated, not decoding.

Thumbnails therefore land on keyframe boundaries rather than exact times. That
is invisible in a filmstrip (phone keyframes are ~1 s apart) and costs nothing,
because the strip is a navigation aid: trim points and the playhead come from
the track geometry, never from thumbnail positions.

Scale goes **before** the tone-map here, the opposite of the render chain
(§7.1): tone-mapping 64 px thumbnails is about twice as fast as tone-mapping
full frames and is indistinguishable at that size.

Generate from the proxy when one exists, otherwise from the original. Failure
is non-fatal: fall back to a plain track and let the user keep editing. The
cache key must include the rotation, or stale thumbnails come back.
...
## 9.1 Persisted settings

The app writes exactly one thing outside its own folder: `settings.json` in
`app.getPath('userData')`, holding the folder the last video was opened from.

- Recorded in the `media:probe` handler, so it covers every route a file can
  arrive by — drop, dialog, or command line — in one place.
- Read when the open dialog is about to show, never at startup: a remembered
  folder can vanish between sessions, and checking late means a stale value
  can't affect launch.
- The choice itself is `pickStartFolder()` in `src/shared/folder.js`, kept free
  of Electron and fs so it is directly testable. Falls back to the Desktop,
  then home.

Keep it to this. The app is a copy-the-folder-and-run tool; it should not
accumulate state a user can't see or reset by deleting one file.
